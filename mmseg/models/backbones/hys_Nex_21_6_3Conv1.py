...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=-1,
        ...
        self.norm2 = nn.LayerNorm(dim, eps=1e-6)  # important to avoid attention collapsing
        self.norm3 = nn.LayerNorm(dim, eps=1e-6)  # important to avoid attention collapsing
        self.norm4 = nn.LayerNorm(dim, eps=1e-6)  # important to avoid attention collapsing
        self.norm5 = nn.LayerNorm(dim, eps=1e-6)  # important to avoid attention collapsing
        self.norm6 = nn.LayerNorm(dim, eps=1e-6)  # important to avoid attention collapsing
        if topk > 0:
            self.attn = BiLevelRoutingAttention(dim=dim, num_heads=num_heads, n_win=n_win, qk_dim=qk_dim,
                                                ...
                                                auto_pad=auto_pad)
            self.attn2=WindowAttention(dim=dim, n_win=n_win, num_heads=num_heads)
        elif topk == -1:
        ...
        if layer_scale_init_value > 0:
            ...
            self.gamma3 = nn.Parameter(1 * torch.ones((dim)), requires_grad=True)
            self.gamma11 = nn.Parameter(1 * torch.ones((dim)), requires_grad=True)
            self.gamma22 = nn.Parameter(1* torch.ones((dim)), requires_grad=True)
            self.gamma33 = nn.Parameter(1* torch.ones((dim)), requires_grad=True)
        else:
            self.use_layer_scale = False
        self.pre_norm = pre_norm
        self.C1=nn.Conv2d(dim,dim,3,1,1)
        self.C2=nn.Conv2d(dim,dim,5,1,2)
    def forward(self, x):
    ##版本2添加内部的卷积混合，效果不好
            ...
            #         x = xx + self.drop_path(self.gamma1 * self.attn2(x)+self.gamma3* x2)  # (N, H, W, C)
            #         x = x + self.drop_path(self.gamma2 * self.mlp2(x))  # (N, H, W, C)
            #     # else:
            #     #     x = xx + self.drop_path(self.attn2(x)+x2)  # (N, H, W, C)
            #     #     x = xx + self.drop_path(self.mlp2(x)+x2)  # (N, H, W, C)
            # x = x.permute(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W)
            
            # x = x + self.pos_embed(x)
            # x = x.permute(0, 2, 3, 1)
            # xx=x #获得初始的位置新消息来进行残差链接
            # x=self.norm2(x)
            # x = x.permute(0, 3, 1, 2)
            # x3=self.C1(x)
            # # permute to NHWC tensor for attention & mlp
            ...
            # if self.pre_norm:
            #     # if self.use_layer_scale:
            #         x = xx + self.drop_path(self.gamma11 * self.attn(x)+self.gamma33*x3)  # (N, H, W, C)
            #         x = x + self.drop_path(self.gamma22 * self.mlp(x))  # (N, H, W, C)
            #     # else:
            #     #     x = xx + self.drop_path(self.attn(x)+x3) # (N, H, W, C)
            #     #     x = xx + self.drop_path(self.mlp(x)+x3) # (N, H, W, C)W, C)
            # # permute back
            # x = x.permute(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W)
            # return x
    #版本1，原始的全局-局部的结构
        ...
        if self.pre_norm:
            ...
            if self.use_layer_scale: # (N, H, W, C)
        ...
        if self.pre_norm:
            if self.use_layer_scale:
                x = x + self.drop_path(self.gamma1 * self.attn2(self.norm4(x)))  # (N, H, W, C)
                x = x + self.drop_path(self.gamma2 * self.mlp2(self.norm5(x)))  # (N, H, W, C)
            else:
                x = x + self.drop_path(self.attn2(self.norm4(x)))  # (N, H, W, C)
                x = x + self.drop_path(self.mlp2(self.norm5(x)))  # (N, H, W, C)
        else:  # https://kexue.fm/archives/9009
            if self.use_layer_scale:
                x = self.norm4(x + self.drop_path(self.gamma1 * self.attn2(x)))  # (N, H, W, C)
                x = self.norm5(x + self.drop_path(self.gamma2 * self.mlp2(x)))  # (N, H, W, C)
            else:
                x = self.norm4(x + self.drop_path(self.attn2(x)))  # (N, H, W, C)
                x = self.norm5(x + self.drop_path(self.mlp2(x)))  # (N, H, W, C)
        x = x.permute(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W)
        return x   
...
class BiFormer(nn.Module):
    def __init__(self, depth=[3, 4, 8, 3], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.fusion.append(nn.Conv2d(2*embed_dim[0],embed_dim[0],1,1))
        depths = [3, 3, 9, 3]
        for i in range(3):
            # Stage 下采样 + Block
            ...
            self.fusion.append(nn.Conv2d(2*embed_dim[i+1],embed_dim[i+1],1,1))
            ...
            pool_layer = nn.Conv2d(embed_dim[i], embed_dim[i+1], kernel_size=(3, 3), stride=(2, 2), padding=(1, 1))
            if (pe is not None) and (i + 1 in pe_stages):
