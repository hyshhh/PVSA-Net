...
from .hys_Nex_21_6_3Conv1 import BiFormer
# from .biformer_hys_v4 import BiFormer
...
#只有local-vote的逻辑
class BiFormer_Nex3Conv1(BiFormer):
    ...
    def forward_features(self, x: torch.Tensor):
        ...
        for i in range(4):
            ...
            # print(torch.cat([x,cnn_encoder_out],dim=1).shape)
            # print(self.fusion[i](torch.cat([x,cnn_encoder_out],dim=1)).shape)
            out.append(self.extra_norms[i](x))
            

            # out.append(self.extra_norms[i](self.fusion[i](torch.cat([x,cnn_encoder_out],dim=1))))
