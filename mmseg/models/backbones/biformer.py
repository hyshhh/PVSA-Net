...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=-1,
        ...
        self.norm1 = nn.LayerNorm(dim, eps=1e-6)  # important to avoid attention collapsing
        if topk > 0:
            self.attn =ToppAttention(dim=dim, num_heads=num_heads, n_win=n_win, qk_dim=qk_dim,
                                                qk_scale=qk_scale, kv_per_win=kv_per_win,
                                                kv_downsample_ratio=kv_downsample_ratio,
                                                kv_downsample_kernel=kv_downsample_kernel,
                                                kv_downsample_mode=kv_downsample_mode,
...
class BiFormer(nn.Module):
    def __init__(self, depth=[3, 4, 8, 3], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        pool1=nn.Sequential(
                    nn.AvgPool2d(kernel_size=3, stride=2,padding=(1, 1)),  # 第一次平均池化，下采样 ×0.5
                    nn.AvgPool2d(kernel_size=3, stride=2,padding=(1, 1)),  # 第二次平均池化，再下采样 ×0.
                    nn.Conv2d(in_chans, embed_dim[0], kernel_size=(1, 1), stride=(1,1), padding=(0, 0))
        )
        ...
        for i in range(3):
            ...
            downsample_layer2 = nn.Sequential(
                nn.Conv2d(embed_dim[i], embed_dim[i + 1], kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)),
                nn.BatchNorm2d(embed_dim[i + 1]),
                # DepthWiseConvModule(embed_dim[i], embed_dim[i + 1]//2, embed_dim[i + 1],3, 2, 1),
                DepthWiseConvModule(embed_dim[i + 1], 4*embed_dim[i + 1], embed_dim[i + 1],3, 1, 1),
                DepthWiseConvModule(embed_dim[i + 1], 4*embed_dim[i + 1], embed_dim[i + 1],3, 1, 1),
                # nn.BatchNorm2d(embed_dim[i + 1])
            )
            pool1 = nn.Sequential(nn.AvgPool2d(kernel_size=3, stride=2,padding=(1, 1)),
                                  nn.Conv2d(embed_dim[i], embed_dim[i + 1], kernel_size=(1, 1), stride=(1, 1), padding=(0, 0))) # 第一次平均池化，下采样 ×0.5
